package com.company;

public class EmailData {

    public static String subject = "SpamBotEmail";
    public static String content = "Я РОДИЛСЯ!!!11!! ПЫЩЬ ПЫЩЬ АДИН АДИН!!";


    public static  String smtpHost="smtp.rambler.ru";


    public static String address="java-task02@rambler.ru";


    public static  String login="java-task02";
    public static  String password="SimplePassword857";


    public static String smtpPort="25";

}
